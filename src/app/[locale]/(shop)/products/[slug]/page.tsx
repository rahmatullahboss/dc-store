  ...
  const highlightCards = [
    {
      ...
      subtitle: t("features.delivery.desc"),
    },
    {
      icon: ShieldCheck,
      title: t("features.payment.title"),
      subtitle: t("features.payment.desc"),
    },
    {
      icon: ShoppingCart,
      title: t("features.returns.title"),
      subtitle: t("features.returns.desc"),
    },
  ];
              ...
              <div className="grid gap-3 rounded-xl sm:rounded-[2rem] border border-white/60 bg-card/70 p-4 sm:p-6 backdrop-blur">
                ...
                <div className="flex flex-wrap gap-3 text-sm text-muted-foreground">
                  <span className="inline-flex items-center gap-2 rounded-full bg-amber-100 dark:bg-amber-900/50 px-3 py-1 text-amber-700 dark:text-amber-300">
                    ⭐ {t("rated", { rating: averageRating.toFixed(1) })}
                  </span>
              ...
              <div className="sticky top-28 space-y-6">
                ...
                <div className="rounded-xl sm:rounded-[2.75rem] border border-white/60 bg-card/80 p-4 sm:p-8 shadow-2xl shadow-amber-100/70 backdrop-blur">
                  ...
                  <div className="flex items-start justify-between gap-4">
                    ...
                    >
                      {product.quantity && product.quantity > 0 ? t("inStock") : t("outOfStock")}
                    </Badge>
                  </div>
                  ...
                  <div className="mt-6 flex items-end justify-between gap-4">
                    <div>
                      <p className="text-xs uppercase tracking-[0.3em] text-primary">
                        {discountPercentage > 0 ? t("specialOffer") : t("startingFrom")}
                      </p>
                    ...
                    {discountPercentage > 0 && (
                      <span className="rounded-full bg-gradient-to-r from-amber-400 to-rose-400 px-4 py-2 text-xs font-semibold uppercase tracking-[0.25em] text-white shadow-lg">
                        {t("save", { amount: ((product.compareAtPrice || 0) - product.price).toLocaleString() })}
                      </span>
                    )}
                  </div>
                ...
                <div className="space-y-6 text-muted-foreground">
                  <div className="rounded-2xl border border-amber-100 dark:border-amber-900/30 bg-amber-50/50 dark:bg-amber-900/20 p-6">
                    <h4 className="font-semibold text-foreground mb-2">
                      📦 {t("shipping.deliveryInfo")}
                    </h4>
                    <ul className="space-y-2 text-sm">
                      <li>• {t("shipping.deliveryPoints.inside")}</li>
                      <li>• {t("shipping.deliveryPoints.outside")}</li>
                      <li>• {t("shipping.deliveryPoints.free")}</li>
                    </ul>
                  </div>
